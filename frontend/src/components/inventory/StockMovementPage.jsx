...
import { createStockMovement, fetchInventoryItems, fetchStockMovements, fetchSuppliers } from "@/src/api/inventoryApi.js";
...
export default function StockMovementPage({ pageType }) {
  ...
  const [form, setForm] = useState({ item: "", supplier: "", quantity: "", reason: "", notes: "", unitCost: "", lotNumber: "", receivedAt: "", expiryDate: "", allowNegativeStock: false });
  const [submitting, setSubmitting] = useState(false);
  useEffect(() => { Promise.all([fetchInventoryItems({ limit: 100, sortBy: "name", sortOrder: "asc" }), fetchSuppliers()]).then(([itemResponse, supplierRows]) => { setItems(itemResponse.data); setSuppliers(supplierRows); }).catch(() => toast.error("Unable to load stock movement options.")); }, []);
  ...
  const purchaseUnit = selectedItem?.purchaseUnit || selectedItem?.unit;
  const conversionFactor = Number(selectedItem?.purchaseConversionFactor || 1);
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setForm({ item: "", supplier: "", quantity: "", reason: "", notes: "", unitCost: "", lotNumber: "", receivedAt: "", expiryDate: "", allowNegativeStock: false });
  ...
  const columns = useMemo(() => [
    ...
    { key: "batch", label: "Batch / Lot", render: (row) => row.batchAllocations?.map((allocation) => allocation.lotNumber).join(", ") || "—" },
  ...
  return <div className="mx-auto max-w-[1500px]"><PageHeader title={pageType === "stock-in" ? "Stock In" : "Stock Out"} description="Every quantity change creates a permanent transaction with before and after balances." />
    ...
    <div className="grid gap-4 xl:grid-cols-[420px_minmax(0,1fr)]">
      <form onSubmit={submit} className={`${cardClass} h-fit p-5`}>
        <div className="mb-5 flex items-start gap-3"><span className="rounded-xl bg-dune-amber/10 p-2.5 text-dune-amber"><SelectedIcon className="h-5 w-5" /></span><div><h2 className="font-body text-base font-semibold text-white">Record {selectedOption.label}</h2><p className="mt-1 text-xs leading-5 text-neutral-600">{selectedOption.help}</p></div></div>
        <div className="space-y-4">
          <Field label="Inventory item"><DarkSelect required className={inputClass} value={form.item} onChange={(event) => { const itemId = event.target.value; const item = items.find((row) => row._id === itemId); setForm((current) => ({ ...current, item: itemId, supplier: item?.supplier?._id || item?.supplier || "" })); }}><option value="">Choose item</option>{items.map((item) => <option key={item._id} value={item._id}>{item.name} · {formatQuantity(item.currentStock, item.unit)}</option>)}</DarkSelect></Field>
          {selectedItem && <div className="rounded-xl border border-white/10 bg-black/20 p-3 text-xs"><span className="text-neutral-600">Current stock</span><p className="mt-1 text-lg font-semibold text-white">{formatQuantity(selectedItem.currentStock, selectedItem.unit)}</p>{isInbound && <p className="mt-1 text-neutral-500">1 {purchaseUnit} = {conversionFactor} {selectedItem.unit}</p>}</div>}
          <Field label={isAdjustment ? "Verified stock quantity" : isInbound ? `Receive quantity (${purchaseUnit || "purchase unit"})` : "Movement quantity"} hint={isAdjustment ? "This is the new exact stock level, not the difference." : isInbound && selectedItem ? `Adds ${formatQuantity(Number(form.quantity || 0) * conversionFactor, selectedItem.unit)}` : selectedItem ? `Measured in ${selectedItem.unit}` : "Choose an item first"}><input required min="0" step="any" type="number" className={inputClass} value={form.quantity} onChange={(event) => set("quantity", event.target.value)} /></Field>
          {isInbound && <div className="grid gap-4 sm:grid-cols-2">
            <Field label={`Cost / ${purchaseUnit || "purchase unit"} (SAR)`} hint={selectedItem && form.unitCost !== "" ? `${(Number(form.unitCost) / conversionFactor).toFixed(4)} SAR / ${selectedItem.unit}` : "Converted to base-unit cost by the server."}><input min="0" step="0.01" type="number" className={inputClass} value={form.unitCost} onChange={(event) => set("unitCost", event.target.value)} placeholder={selectedItem ? (Number(selectedItem.unitCost || 0) * conversionFactor).toFixed(2) : "0.00"} /></Field>
            <Field label="Supplier"><DarkSelect className={inputClass} value={form.supplier} onChange={(event) => set("supplier", event.target.value)}><option value="">No supplier</option>{suppliers.map((supplier) => <option key={supplier._id} value={supplier._id}>{supplier.name}</option>)}</DarkSelect></Field>
            <Field label="Batch / Lot number" hint="Leave blank to generate one automatically."><input className={inputClass} value={form.lotNumber} onChange={(event) => set("lotNumber", event.target.value.toUpperCase())} placeholder="SUP-LOT-2026-001" /></Field>
            <Field label="Received date" hint="Leave blank to use the current time."><DarkDatePicker type="datetime-local" className={inputClass} value={form.receivedAt} onChange={(event) => set("receivedAt", event.target.value)} /></Field>
            <Field label="Expiry date"><DarkDatePicker className={inputClass} disabled={!selectedItem?.tracksExpiry} value={form.expiryDate} onChange={(event) => set("expiryDate", event.target.value)} /></Field>
          </div>}
          <Field label="Reason"><input required className={inputClass} value={form.reason} onChange={(event) => set("reason", event.target.value)} placeholder="Delivery, kitchen use, spoilage…" /></Field>
          <Field label="Notes"><textarea className={textareaClass} value={form.notes} onChange={(event) => set("notes", event.target.value)} /></Field>
          {!isInbound && <label className="flex items-start gap-3 rounded-xl border border-red-500/15 bg-red-500/5 p-3 text-xs text-neutral-400"><input type="checkbox" checked={form.allowNegativeStock} onChange={(event) => set("allowNegativeStock", event.target.checked)} className="mt-0.5 h-4 w-4 accent-red-500" /><span><strong className="block text-red-300">Explicitly allow negative stock</strong>Use only when an authorized backdated correction must go below zero.</span></label>}
          <Button type="submit" className="w-full" disabled={submitting}>{submitting ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}Record movement</Button>
        </div>
      </form>
