...
export default function WasteRecordForm({ items, submitting, onSubmit }) {
  ...
  return <form onSubmit={submit} className="space-y-4">
    ...
    <Field label="Inventory item"><DarkSelect required className={inputClass} value={form.item} onChange={(event) => set("item", event.target.value)}><option value="">Choose item</option>{items.map((item) => <option key={item._id} value={item._id}>{item.name} · {item.sku}</option>)}</DarkSelect></Field>
    ...
    <div className="grid gap-4 sm:grid-cols-2"><Field label={`Quantity lost${selected ? ` (${selected.unit})` : ""}`}><input required type="number" min="0.000001" max={selected?.currentStock} step="any" className={inputClass} value={form.quantity} onChange={(event) => set("quantity", event.target.value)} /></Field><Field label="Reason"><DarkSelect className={inputClass} value={form.reasonCode} onChange={(event) => set("reasonCode", event.target.value)}>{reasons.map((reason) => <option key={reason} value={reason}>{humanize(reason)}</option>)}</DarkSelect></Field></div>
    <Field label="Recorded date and time" hint="Leave blank to use the current time."><DarkDatePicker type="datetime-local" className={inputClass} value={form.occurredAt} onChange={(event) => set("occurredAt", event.target.value)} /></Field>
