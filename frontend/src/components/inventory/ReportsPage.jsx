...
export default function ReportsPage() {
  ...
  return <div className="mx-auto max-w-[1600px]"><PageHeader title="Inventory Reports" description="Operational and financial inventory reporting. All monetary values are SAR." actions={<><Button variant="secondary" onClick={() => reload()}><RefreshCw className="h-4 w-4" />Refresh</Button><Button onClick={exportCsv}><Download className="h-4 w-4" />Export CSV</Button></>} /><div className="mb-4 flex gap-2 overflow-x-auto pb-1">{reports.map(([value, label]) => <button key={value} type="button" onClick={() => { setType(value); setPage(1); }} className={`whitespace-nowrap rounded-xl border px-3 py-2 text-xs font-semibold transition ${type === value ? "border-dune-amber/40 bg-dune-amber/10 text-dune-amberLight" : "border-white/10 bg-white/[0.025] text-neutral-500 hover:text-white"}`}>{label}</button>)}</div><section className={cardClass}><div className="flex flex-col gap-3 border-b border-white/10 p-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="flex items-center gap-2 text-sm font-semibold text-white"><BarChart3 className="h-4 w-4 text-dune-amber" />{reports.find(([value]) => value === type)?.[1]}</p><p className="mt-1 text-xs text-neutral-600">Live database report · SAR</p></div><div className="flex flex-wrap gap-2">{dateUseful && <><label className="text-[0.65rem] text-neutral-600">From<DarkDatePicker className={`${inputClass} mt-1 w-40`} value={from} onChange={(event) => { setFrom(event.target.value); setPage(1); }} /></label><label className="text-[0.65rem] text-neutral-600">To<DarkDatePicker className={`${inputClass} mt-1 w-40`} value={to} onChange={(event) => { setTo(event.target.value); setPage(1); }} /></label></>}{type === "supplier" && <label className="text-[0.65rem] text-neutral-600">Supplier<DarkSelect className={`${inputClass} mt-1 w-56`} value={supplier} onChange={(event) => { setSupplier(event.target.value); setPage(1); }}><option value="">All suppliers</option>{suppliers.map((row) => <option key={row._id} value={row._id}>{row.name}</option>)}</DarkSelect></label>}</div></div>{loading ? <LoadingState /> : <DataTable columns={columns} rows={data?.data} empty={<EmptyState title="No report data" description="No records match the selected report and filters." />} />}<Pagination pagination={data?.pagination} onPageChange={setPage} /></section></div>;
}
